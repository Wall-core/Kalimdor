...
bool GossipSelect_EnchantNPC(Player* player, Creature* creature, uint32 sender, uint32 action)
{
    ...
    if (action < 20)
    {
        ...
        case EQUIPMENT_SLOT_MAINHAND:
            ...
            player->ADD_GOSSIP_ITEM(5, "Fiery Blaze", GOSSIP_SENDER_MAIN, WEP_BLAZE);
            ...
            player->ADD_GOSSIP_ITEM(5, "Unholy", GOSSIP_SENDER_MAIN, WEP_UNHOLY);
            player->ADD_GOSSIP_ITEM(5, "Weapon Chain", GOSSIP_SENDER_MAIN, WEP_CHAIN);
            break;
        case EQUIPMENT_SLOT_OFFHAND:
            ...
            player->ADD_GOSSIP_ITEM(5, "Thorium Spike", GOSSIP_SENDER_MAIN, OFFHAND_SPIKE);
            break;
        }
    ...
    {
        if (!player->HasItemCount(40039, 1, false))
        {
            player->GetSession()->SendNotification("Requires PvP Tokens");
            player->CLOSE_GOSSIP_MENU();
            return true;
        }
        ...
        case OFFHAND_FROSTRES:
            ...
            if (item && (item->GetProto()->Class != ITEM_SUBCLASS_ARMOR_SHIELD))
            {
                player->GetSession()->SendNotification("Requires a Shield");
                player->CLOSE_GOSSIP_MENU();
                return true;
            }
        ...
        player->DestroyItemCount(40039, 1, true);
        player->CLOSE_GOSSIP_MENU();
    }
    return true;
}
